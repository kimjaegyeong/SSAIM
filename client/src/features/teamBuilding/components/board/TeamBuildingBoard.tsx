...
const TeamBuildingBoard: React.FC = () => {
    ...
    const filteredData = data.filter((item) => {
        ...
        return regionMatch && membersMatch && positionMatch;
    });
    ...
    const renderPagination = () => {
        ...
        return (
            <div className={`${styles.pagination}`}>
                {currentPage > 1 && (
                    <button onClick={() => handleClick(currentPage - 1)}>이전</button>
                )}
                {pageNumbers}
                {currentPage < totalPages && (
                    <button onClick={() => handleClick(currentPage + 1)}>다음</button>
                )}
            </div>
        );
    };
            ...
            <div className={styles.board}>
                <div className={styles.boardHeader}>
                    <div className={styles.filterSection}>
                        <DropDown options={regionOptions} onSelect={(option) => handleSelect('region', option)} placeholder="지역" />
                        <DropDown options={memberOptions} onSelect={(option) => handleSelect('members', option)} placeholder="인원" />
                        <DropDown options={positionOptions} onSelect={(option) => handleSelect('position', option)} placeholder="직무" />
                    </div>
